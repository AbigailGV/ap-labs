             ...
             int fileflags, struct FTW *pfwt)
{
  if (fileflags == FTW_DP && pfwt->level < 2)
  {
    ...
    subdirs[lastDire++] = strcat(name, "/");
  }
  return 0;
}

void refreshDirs(){
  char *root = subdirs[lastDire - 1];
  lastDire = 0;
  nftw(root, nftwfunc, fd_limit, flags);

  for (int j = 0; j < lastDire; j++) {
...
static void displayInotifyEvent(int inotifyInstance, struct inotify_event *i)
{
  if (i->mask & IN_ISDIR)
  {
   ...
   if (i->mask & IN_MOVED_FROM)
      lasttNamee = strdup(i->name);
    if (i->mask & IN_MOVED_TO)
      infof("- [Directory - Rename] - %s%s/ -> %s%s/\n", descriptors[i->wd], lasttNamee, descriptors[i->wd], i->name);
    if (i->mask & IN_DELETE)
...
int main(int argc, char *argv[])
{
  ...
  for (;;)
  {
    numbRead = read(inotifyFd, buf, BUF_LEN);
    if (numbRead == 0)
